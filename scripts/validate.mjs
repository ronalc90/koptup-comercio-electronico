...
(() => {
  ...
  for (const m of ['003_strict_rls', '004_billing', '005_fix_product_limit_race', '006_audit_log', '007_alerts', '008_fix_trigger_security_definer', '009_business_constraints', '010_constraints_followup', '011_product_image_url']) {
    if (!read(`migrations/${m}.sql`)) fails.push(`falta ${m}.sql`);
  }
  record('4. migraciones', fails.length === 0, fails.length ? `falta: ${fails.join(', ')}` : 'estructura completa (002–011)');
})();
